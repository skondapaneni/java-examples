...
public final class MurmurHash3 {
  ...
  /** Returns the MurmurHash3_x86_32 hash. */
  public static int murmurhash3_x86_32(byte[] data, byte[] datamask, 
		  int offset, int len, int seed) {
    ...
    for (int i=offset; i<roundedEnd; i+=4) {
      ...
      int k1 = (data[i] & datamask[i] & 0xff) |
    		  ((data[i+1] & datamask[i+1] & 0xff) << 8) | 
    		  ((data[i+2] & datamask[i+2] & 0xff) << 16) | 
    		  ((data[i+3] & datamask[i+3]) << 24);
      k1 *= c1;
      k1 = (k1 << 15) | (k1 >>> 17);  // ROTL32(k1,15);
      k1 *= c2;
