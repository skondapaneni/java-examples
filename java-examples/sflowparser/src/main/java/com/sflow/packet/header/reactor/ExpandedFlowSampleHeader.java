...
public class ExpandedFlowSampleHeader {
     ...
     public static ExpandedFlowSampleHeader parse(byte[] data, int offset, SFlowHeader sflowHeader) throws HeaderParseException {
          try {
...
//               System.out.println(Thread.currentThread() + " notifying expaned flow Record " + 
//                         efsh.getSequenceNumber() + " " + efsh.getNumberFlowRecords());
               
               sflowHeader.getCollector().getRawPacketRecordBus().notify(
                         ExpFlowSampleHeaderConsumer.expandedFlowSampleEvent,
                    Event.wrap(efsh) );
