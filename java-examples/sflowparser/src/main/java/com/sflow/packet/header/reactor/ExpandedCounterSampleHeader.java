...
public class ExpandedCounterSampleHeader {
	...
	public static ExpandedCounterSampleHeader parse(byte[] data, int offset, SFlowHeader sflowHeader)
		...
		try {
			...
			sflowHeader.getCollector().getCounterRecordBus()
					.notify(ReactorExpCounterSampleHeaderConsumer.expandedCounterSampleHeaderEvent, Event.wrap(ecsh));
